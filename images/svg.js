...
get_cookie()
{
  ...
  if(c == null)
    return "";
  var results = c.match('assp_gr_stat=(.*?)(;|$)' );
...
set_cookie(value)
{
  parent.document.cookie="assp_gr_stat="+escape(value);
}
...
function
showOtherPoints(d, lname, currval, maxval)
{
  for(var i=0; i < 9; i++) {
    var name ="line_"+i;
    if (name != lname) {
      var el = new Array();
      var el = d.getElementsByName(name);
      for (var j = 0; j < el.length; ++j) {
        var h = parseFloat(el[j].getAttribute("y_h"));
        el[j].setAttribute("transform", "translate(0,"+h*(1-currval)+") "+
                                        "scale(1,"+currval+")");
      }
    }
  }
...
{
  var t=evt.target;
  var y_offset = parent.document.getElementById('svggraphic').offsetTop;
  var y_mul = parseFloat(t.getAttribute("y_mul"));
  var y_h   = parseFloat(t.getAttribute("y_h"));
  var y_min = parseFloat(t.getAttribute("y_min"));
  var y_org = parseInt(((y_h-evt.clientY+y_offset)/y_mul)+0.5)+' / '+parseInt(((y_h-evt.clientY+y_offset)/y_mul)+y_min+0.5);
...
svg_click_conf(evt)
{
  ...
  var y_h   = parseFloat(t.getAttribute("y_h"));
  var y_min = parseFloat(t.getAttribute("y_min"));
  var y_org = parseInt(((y_h-evt.clientY+y_offset)/y_mul)+0.5);

  var x_mul = parseFloat(t.getAttribute("x_mul"));
  var x_off = parseFloat(t.getAttribute("x_off"));
  var x_min = parseFloat(t.getAttribute("x_min"));
  var conf  = parseFloat( Math.pow(confTop , ( confRev - ((evt.clientX-x_min+x_off)/x_mul))) ).toFixed(5);
  
  var tl = evt.target.ownerDocument.getElementById('svg_title');
  tl.firstChild.nodeValue = t.getAttribute("title")+" confidence: "+conf+" count: "+y_org;
}
