This should be a short whitepaper to configure and use SPAMBOX with any supported database.
...
spamdb you need a SPAMBOX version 1.3.6 or above! There has to be a file spambox_db_import.cfg in SPAMBOX root directory!!!!
...
SPAMBOX requires permanent opened connections to the database listener. If you set a maximum allowed concurrent connections
...
- mydb:
This database must exist before starting SPAMBOX, necessary tables will be created automatically into this database.
...
This is a good time to restart SPAMBOX - you have to do this any time, you have changed any database related parameters listed
above!
How ever - SPAMBOX should now be ready to work. If you have made an upgrade form an earlyer version, you can import your old
...
will be done by SPAMBOX every time it starts and it finds a import-file in the import-directory.
You need to configure "importDBDir" and you have to copy all files you want to import in to this directory. Now rename this
files to *.add or *.rpl what ever you want SPAMBOX to do - an ADD or a REPLACE records in to the database.
...
At this time all functions of SPAMBOX are well tested with MySQL(ODBC/direct),MSSQL(ODBC/ADO) and Oracle(ODBC/direct),
but it should work with any other database, as long as you have a perl driver for that. Only if you want to use the
import function with you database, you may get DBI errors. In this case the file spambox_db_import.cfg should be modified
for your database. Please change SPAMBOX in to debug mode, stop SPAMBOX, rename the maillog.txt,
prepaire the import directory and start SPAMBOX. Please send the following informations to SPAMBOX group:
database type and version - and the maillog.txt with the errors.
If you have some knowledge in SQL you can try to add a section for your database to the spambox_db_import.cfg file!

If you want to replicate databases between several SPAMBOX installations, you have to make sure that the replication processes ignoring duplicate records. 
...
because some MSSQL versions will create an extra identity column with that attribute, which will lead in to database errors in SPAMBOX. 
...
- Set 'preventBulkImport' to on - otherwise the import procedure of SPAMBOX will destroy replication attributes/constraints of the DB tables. 
If you want to use the BulkImport, you must edit the file 'spambox_db_import.cfg' by changing the statements for 
