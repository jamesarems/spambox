#!/usr/local/bin/perl
###############################################################################################################################
# spambox-monitor 1.11
# --------------
# can be used instead of a syslog-server to have a local or remote monitor of spambox
# 1. configure the syslog option in spambox
# 2. copy this script to your monitor system, if you are using this script on the local spambox system just copy
#    this script in to the spambox directory
# 3. be sure there is no syslog daemon running on the system - or use an other port (1514 instead of 514)
# 4. run the script - if started on the local spambox system the script will try to find the SysLogPort
#    in spambox.cfg
#    if started on a remote system, define the listenport as first parameter - like
#    perl spambox-monitor.pl 1514 or the default port 514 will be used
###############################################################################################################################
...
##############################################
 ...
 my $sep = ($^O ne 'MSWin32') ? "'" : '"';
 my $killcmd = "$^X -e $sep kill 9, SPAMBOXPID $sep";
 my $startcmd = 'cmd.exe /C net start SPAMBOXSMTP';
 my $startwait = 120;
...
# try opening an UDP socket
 ...
 print "\nspambox-monitor.pl is listening for UDP-connections on port $port\n\n";
 print "found syslog configuration in $cfg:\nIP     : $sysLogIp\nPort   : $port\nsysLog : $sysLog\n\n" if $sl;
 print "monitoring spambox.pl at PID $pid for an over two minutes keep alive\n";
 print "will use the command <$killcmd> to kill spambox\n" if $killcmd;
 print "will use the command <$startcmd> to start spambox\n" if $startcmd;
 print "expected hardbeat time is $hangtime seconds\n";
 print "will wait $startwait seconds before restarting spambox\n";
...
# listen on udp-port - format the message to looks like spambox log output
...
# monitor spambox.pl for keep alive
 while (1) {
  ...
  if (! $fh) {
      ...
      print "!!! SPAMBOX seems not to be alive for over two minutes - will try to kill and restart\n !!!";
      open(my $F, '<', $pidfile);
      $pid = <$F>;
      close $F;
      $pid =~ s/\r|\n//go;
      if ($pid && (kill 0, $pid)) {
          my $cmd = $killcmd;
          $cmd =~ s/SPAMBOXPID/$pid/og;
          system($cmd);
      }
      sleep 10;
      system($startcmd) if ($startcmd);
      $lastread = time + $startwait;   # give spambox 4 minutes to startup
      next;
  }
